...
func MakeRange(r *lbv1.Range) (*allocator.Range, error) {
	...
	// The rangeStart defaults to “.1” IP inside of the “subnet” block.
	if r.RangeStart == "" {
		start = ip.NextIP(ipNet.IP)
	} else {
	...
	// The rangeEnd defaults to “.254” IP inside of the “subnet” block for ipv4, “.255” for IPv6.
	if r.RangeEnd == "" {
		end = lastIP(*ipNet)
	} else {
	...
	// The gateway defaults to “.1” IP inside of the “subnet” block.
	// The gateway will be skipped during allocation.
	if r.Gateway == "" {
		gateway = ip.NextIP(ipNet.IP)
	} else {
