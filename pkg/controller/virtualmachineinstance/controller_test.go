...
func TestOnVmiChanged_SkipProcessing(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, ok := h.getHostnameFromVMIToHostnameMap(tc.vmi); ok {
				t.Fatalf("expected vmi is not stored on local map but stored")
			}
		})
	}
}
...
func TestOnVmiChanged_HostnameLookup(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "5. agent ready, hostname matches a node, but node is already pointed to another VMI, return conflict",
			guestOsInfoRet: kubevirtv1.VirtualMachineInstanceGuestAgentInfo{Hostname: targetNodeName},
			initialNodes:   initialNodes,
			initialNodeNameMaps: map[string]string{
				targetNodeName: "other-vmi-name",
			},
			expectError:   true,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				if _, ok := h.getVMFromNodeToVMNameMap(tc.targetNodeNameToCheck); !ok {
					t.Fatalf("expected to find node %s in map, but does not find", tc.targetNodeNameToCheck)
				}
			}
		})
	}
...
func TestOnVmiChanged_LocalLookupMapUpdates(t *testing.T) {
	namespace := "default"
	vmiName := "vmi-example"
	targetNodeName := "saved-node-name"
	clusterName := "test-cluster"
	...
	initialNodes := map[string]*corev1.Node{
		targetNodeName: {ObjectMeta: metav1.ObjectMeta{Name: targetNodeName}},
		"fresh-node":   {ObjectMeta: metav1.ObjectMeta{Name: "fresh-node"}},
	}

	vmiUID := types.UID("current-vmi-uid-999")
	staleUID := types.UID("stale-vmi-uid-111")
	...
	t.Run("1. Local lookup map hits with matching UID avoids GuestOsInfo call", func(t *testing.T) {
		vmi := baseVMI.DeepCopy()
		h := &Handler{
			namespace: namespace,
	...
	t.Run("1. Local lookup map hits UID mismatch and updates", func(t *testing.T) {
		...
		h := &Handler{
			...
			kubevirtClient: &FakeKubevirtClient{
				VMIHandler: func(ns string) kubecli.VirtualMachineInstanceInterface {
					return &FakeVMIInterface{
						GuestOsInfoFunc: func(ctx context.Context, name string) (kubevirtv1.VirtualMachineInstanceGuestAgentInfo, error) {
							guestAgentCalled = true
							return kubevirtv1.VirtualMachineInstanceGuestAgentInfo{Hostname: "fresh-node"}, nil
						},
					}
				},
			},
		...
		// Pre-populate cache with a STALE/mismatched UID (simulating VMI recreation)
		h.vmiToHostname.Store(vmiName, vmiCacheInfo{
			hostname: targetNodeName,
			uid:      staleUID,
		})
		...
		_, err := h.OnVmiChanged("", vmi)
		if err != nil {
			t.Fatalf("expected no error on cache refresh, got: %v", err)
		}

		if !guestAgentCalled {
			t.Fatalf("expected GuestOsInfo to be called due to UID mismatch cache invalidation")
		}

		// Verify new hostname "fresh-node" was stored and used
		if _, ok := h.getVMFromNodeToVMNameMap("fresh-node"); !ok {
			t.Fatalf("expected to find refreshed node 'fresh-node' in map")
		}

		// Verify cache was updated with the fresh hostname and current VMI UID
		hostname, ok := h.getHostnameFromVMIToHostnameMap(vmi)
		if ok {
			if hostname != "fresh-node" {
				t.Fatalf("expected cache to be updated with 'fresh-node' and current UID, got %s", hostname)
			}
		} else {
...
func TestOnVmiChangedNetworkMapping1(t *testing.T) {
	...
	// Fully qualifying VMI with spec networks AND status interfaces populated
