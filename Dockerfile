...
# controller-gen is moved to go.mod; bump it there
# goimports is removed, in this repo, `make generate` does not need `goimports` to do additionaly formatting work
...
WORKDIR /go/src/github.com/harvester/harvester-cloud-provider
...
COPY --from=build /go/src/github.com/harvester/harvester-cloud-provider/bin/ /bin/
